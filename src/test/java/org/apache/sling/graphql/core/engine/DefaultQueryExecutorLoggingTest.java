...
public class DefaultQueryExecutorLoggingTest extends ResourceQueryTestBase {
    ...
    public void verifyNoSchemaLogged() {
        capture.list.stream()
                ...
                .forEach(event ->
                        assertEquals("Expecting schema marker at DEBUG level only", Level.DEBUG, event.getLevel()));
    }
    ...
    public void basicTest() throws Exception {
        ...
        capture.assertContains(Level.DEBUG, TEST_SCHEMA_MARKER, "Executing query");
    }
    ...
    private void assertSchemaFailure(String... selectors) {
        ...
        capture.assertContains(Level.INFO, "Schema provider Exception");
        capture.assertContains(
                Level.ERROR,
                "Query failed for Resource " + resource.getPath(),
                "query=" + query,
