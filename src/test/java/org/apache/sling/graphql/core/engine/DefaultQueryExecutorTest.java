...
public class DefaultQueryExecutorTest extends ResourceQueryTestBase {
    ...
    protected void setupAdditionalServices() {
        ...
        TestUtil.registerSlingDataFetcher(
                context.bundleContext(), "combined/fetcher", new EchoDataFetcher(dataCombined));
    }
    ...
    public void digestFieldsTest() throws Exception {
        ...
        assertThat(
                json,
                hasJsonPath("$.data.currentResource.resourceTypeMD5", equalTo("md5#resourceType#" + resourceTypeMD5)));
    }
    ...
    public void dataFetcherFailureTest() {
        try {
            ...
            assertThat(
                    rex.getMessage(),
                    containsString(
                            "Error: type=DataFetchingException; message=Exception while fetching data (/currentResource/failure) : FailingDataFetcher"));
        }
    }
    ...
    public void invalidQueryTest() {
        ...
        assertTrue(
                "Wrong response, found errors: '" + errors + "'",
                errors.contains(
                        "Error: type=ValidationError; message=Validation error (FieldUndefined@[currentRsrc]) : Field 'currentRsrc' in type 'Query' is undefined; location=1,3;"));
    }
    ...
    public void invalidFetcherNamesTest() {
        context.registerService(
                ...
                new MockSchemaProvider("failing-fetcher-schema"),
                Constants.SERVICE_RANKING,
                Integer.MAX_VALUE);
        final ServiceRegistration<?> reg =
                TestUtil.registerSlingDataFetcher(context.bundleContext(), "missingSlash", new EchoDataFetcher(42));
        try {
            ...
            assertThat(
                    json, hasJsonPath("$.errors[0].extensions.exception", is(SlingGraphQLException.class.getName())));
        } finally {
            reg.unregister();
        }
    }
    ...
    public void invalidTypeResolverNamesTest() {
        context.registerService(
                SchemaProvider.class,
                new MockSchemaProvider("failing-type-resolver-schema"),
                Constants.SERVICE_RANKING,
                Integer.MAX_VALUE);
        final ServiceRegistration<?> reg =
                TestUtil.registerSlingTypeResolver(context.bundleContext(), "missingSlash", new DummyTypeResolver());
        try {
            final String json = queryJSON("{ currentResource { missingSlash } }", new String[] {});
            assertThat(
                    json,
                    hasJsonPath("$.errors[0].message", containsString("Invalid type resolver name missingSlash")));
            assertThat(
                    json, hasJsonPath("$.errors[0].extensions.exception", is(SlingGraphQLException.class.getName())));
        } finally {
            reg.unregister();
    ...
    public void unionQueryTest() throws Exception {
        final String json = queryJSON(
                "{ unionQuery { characters { ... on Human { name address }  ... on Droid { name primaryFunction } } } }");
    ...
    public void interfaceQueryTest() throws Exception {
        final String json = queryJSON(
                "{ interfaceQuery { characters { id ... on Human { name address }  ... on Droid { name primaryFunction } } } }");
    ...
    public void selectionSetTest() throws Exception {
        queryJSON(
                "{ combinedFetcher { boolValue resourcePath aTest { boolValue test resourcePath } allTests { boolValue test resourcePath } unionTest { ... on Human { id address } ... on Droid { id primaryFunction } } interfaceTest { id ... on Human { address } ... on Droid { primaryFunction } } } }");

        // retrieve the service used
        ServiceReference<?>[] serviceReferences = context.bundleContext()
                .getServiceReferences(SlingDataFetcher.class.getName(), "(name=combined/fetcher)");
        EchoDataFetcher echoDataFetcher =
                (EchoDataFetcher) context.bundleContext().getService(serviceReferences[0]);
        ...
        String[] expectedFieldNames = new String[] {"boolValue", "resourcePath", "aTest", "unionTest", "interfaceTest"};
        final List<SelectedField> selectionSetFields = selectionSet.getFields();
        for (String expectedFieldname : expectedFieldNames) {
            assertTrue(
                    "Failed to find expected field name: '" + expectedFieldname + "'",
                    selectionSetFields.stream().anyMatch(f -> expectedFieldname.equals(f.getName())));
        }

        // In new graphql-java qualified field names are streamlined and so we don't have inlined sub types (inlined
        // fragments)
        ...
        String[] expectedSubFieldNames = new String[] {"test", "boolValue", "resourcePath"};
        ...
        for (Map.Entry<String, List<String>> entry : expectedObjectTypes.entrySet()) {
            ...
            assertTrue(
                    "Field did not contain the expected Object Type Names: " + field.getName(),
                    CollectionUtils.isEqualCollection(entry.getValue(), field.getObjectTypeNames()));
        }
    }
    ...
    public void testCachedTypeRegistry() throws IOException {
        ...
        TypeDefinitionRegistry registry1 = queryExecutor.getTypeDefinitionRegistry(
                schemaProvider.getSchema(resource, selectors), resource, selectors);
        TypeDefinitionRegistry registry2 = queryExecutor.getTypeDefinitionRegistry(
                schemaProvider.getSchema(resource, selectors), resource, selectors);
        ...
        context.registerService(
                SchemaProvider.class,
                new MockSchemaProvider("test-schema-selected-foryou"),
                Constants.SERVICE_RANKING,
                Integer.MAX_VALUE);
        TypeDefinitionRegistry registry3 = queryExecutor.getTypeDefinitionRegistry(
                schemaProvider.getSchema(resource, selectors), resource, selectors);
        TypeDefinitionRegistry registry4 = queryExecutor.getTypeDefinitionRegistry(
                schemaProvider.getSchema(resource, selectors), resource, selectors);
        assertEquals(registry1, registry2);
    ...
    public void testTypeRegistryWithTheCacheDisabled() throws IOException {
        ...
        DefaultQueryExecutor queryExecutor = (DefaultQueryExecutor)
                context.registerService(QueryExecutor.class, new DefaultQueryExecutor(), properties);
    ...
    public void testBooleanVariable() throws Exception {
        ...
        final String json =
                queryJSON("query($boolVar: Boolean) { lazyQuery(boolVar: $boolVar) { cheapCount }}", variables);
    ...
    public void testFloatVariable() throws Exception {
        ...
        final String json =
                queryJSON("query($floatVar: Float) { lazyQuery(floatVar: $floatVar) { cheapCount }}", variables);
        assertThat(json, hasNoJsonPath("$.errors"));
    }
}
